...
### 8.4 Required Access-Control Implementation Rules

The following rules are mandatory across the console app, API layer, background jobs, workflows, and every module:

- Every permission key used by a module MUST be declared in that module's manifest. The manifest is the source of truth for permission names, labels, menus, routes, settings, and capability registration.
- Every server-side entry point MUST enforce authorization itself: App Router pages, route handlers, Server Actions, event handlers, workflow steps, and background jobs. UI checks do not replace server checks.
- Client-side permission checks such as hidden buttons, disabled actions, filtered menus, and `usePermission` are convenience only. The server must repeat the real authorization decision.
- Never grant access by role name, menu visibility, tenant slug, client-provided flags, or URL shape. Use permissions, tenant scoping, record rules, field security, and explicit ownership checks.
- Every query and mutation MUST be tenant-scoped before any business filtering.
- Ownership-based access MUST be enforced in services or queries, not implied by the UI.
- Sensitive fields MUST use field-level security instead of scattered ad hoc checks.
- Super-admin bypasses MUST live in one central authorization layer only. Do not duplicate super-admin shortcuts throughout feature code.
- Permission errors returned to clients MUST stay generic and safe. Never expose stack traces, SQL details, storage keys, or internal implementation notes.

### 8.5 Access-Control Design Checklist

Before a feature is considered complete, the implementation MUST answer all of the following:

1. What permission key authorizes read access?
2. What permission key authorizes create, update, delete, export, approve, or any privileged action?
3. Is tenant scoping enforced on every query and mutation?
4. Does row-level access require ownership, assignment, team membership, or another record rule?
5. Do any fields require read/write restrictions?
6. Does the server re-check the same rule the UI is presenting?
7. Is the action audited if it mutates business data, files, settings, permissions, or module state?

### 8.6 File Access Control Policy

The documents system is part of the platform security model and MUST follow these rules everywhere:

- File management is owner-based by default. Non-super-admin users may list, manage, delete, relink, and update metadata only for files they uploaded unless a stricter policy is defined.
- Linked-file consumption is visibility-based and context-based. Access to a linked file MUST be evaluated using effective visibility (`internal`, `tenant`, `public`) plus the linked resource context.
- Raw file UUIDs are not sufficient authorization for non-owners. Non-public file access MUST be tied to a valid resource context or an explicit file-management permission path.
- Direct storage provider URLs, bucket keys, and unsigned object paths MUST never be exposed to the browser or stored as public integration contracts.
- Any feature rendering a linked file MUST use a context-aware file URL generated by the shared document helper or shared documents UI abstraction.
...
### 12.4 Central File Manager Rules

The `documents` module is the only approved file-management system for SBC Core. The following rules are mandatory across the whole program:

- All uploads, previews, downloads, deletions, and linked-document flows MUST go through the central documents module and its shared APIs or shared UI abstractions.
- Modules MUST NOT store raw file bytes, ad hoc upload paths, storage-provider URLs, or object keys in their own tables.
- Business records may store only approved document references: structured links, stable document IDs, or derived URLs produced by the documents layer.
- Any record that uses a file MUST be linked through structured metadata: `resource_module`, `resource_type`, `resource_id`, `field_name`, `link_label`, `visibility`, and `sort_order`.
- Every module attaching files to records MUST use stable `resource_module/resource_type/field_name` values consistently in pages, actions, jobs, and API handlers.
- File previews rendered inside forms, tables, cards, detail pages, or dashboards MUST use context-aware file URLs so access checks remain enforceable at request time.
- Public file access is opt-in and should be rare. The default visibility for linked files MUST be `internal` unless there is a documented product requirement for `tenant` or `public`.
- Upload flows SHOULD provide contextual defaults for `folder`, `module_name`, and tags so assets remain searchable and traceable across the platform.
- Reusing an existing file from the central library is preferred over duplicate uploads when the same asset is referenced by multiple records.
- File deletion MUST retire metadata and the stored object through the documents service. No module may delete storage objects directly.

### 12.5 File Manager Integration Pattern For Other Modules

Every module integrating with documents MUST follow this pattern:

1. Define the business record that owns the relation, for example `crm.customer`, `insurance.policy`, or `hr.employee`.
2. Choose a stable `field_name` for each attachment slot, for example `avatar`, `contract`, `policy_document`, or `evidence`.
3. Upload or select files through the shared file manager or shared file picker.
4. Persist the relation through the documents link layer instead of embedding storage details in the business table.
5. Render the file back through a context-aware URL that includes the resource identity.
6. Enforce business-record permissions separately from file visibility. Linking a file to a record does not replace the record's own authorization rules.
...
### 23.4 Required Module Construction Workflow

Every new module MUST be built in the following order:

1. Scaffold the module package and create `manifest.ts`, `index.ts`, `api/`, `services/`, `db/`, `hooks/`, and `migrations/`.
2. Define module identity, dependencies, permissions, menus, routes, settings, events, and lifecycle hooks in the manifest before writing feature-specific UI.
3. Create database schema files using module-prefixed table names and shared base columns.
4. Add forward-only migrations for schema changes. Schema edits without migrations are not allowed.
5. Implement service-layer logic first, then route handlers or Server Actions, then UI.
6. Enforce tenant scoping, permissions, record rules, and audit logging as part of the first implementation, not as cleanup later.
7. If the current delivery phase still uses a temporary bootstrap or manual registration path, that registration MUST be updated in the same change set. Temporary bootstrap wiring is not optional while dynamic discovery is incomplete.
8. Validate the full slice with typecheck, tests where available, and at least one behavior-scoped verification of install or runtime registration.

### 23.5 Required Module Install Workflow

Installing a module MUST follow the lifecycle contract defined in this document and must never be treated as a copy-paste code drop.

Required install order:

1. Resolve dependencies and conflicts.
2. Register the manifest and make the module discoverable by the active runtime.
3. Run module migrations.
4. Register permissions.
5. Register menus, routes, settings, events, slots, and workflow steps.
6. Run install hooks and optional seed data.
7. Mark the module installed only after all previous steps succeed.

Install rules:

- A module is not considered installed until migrations and permission registration have completed successfully.
- Module routes or menus MUST NOT be exposed before install prerequisites complete.
- Partial install state must be recoverable; failures must leave the system in a known state.
- Every installable module MUST be removable without leaving hardcoded dependencies behind in core code or other modules.

### 23.6 Required Module Update Workflow

Updating a module MUST preserve existing tenant data and runtime contracts.

Required update rules:

- Every released module change that alters schema, settings, permissions, routes, documents integration, or event contracts MUST be versioned and reviewed as an upgrade path, not just as a diff.
- Schema changes MUST ship with migrations. Never edit production table shape in place without a migration.
- New permissions MUST be additive unless there is a documented migration plan for renamed or removed permissions.
- Route, menu, and settings changes MUST remain backward-compatible across the upgrade step or include an explicit remediation path.
- File attachment behavior changes MUST preserve existing `documents_links` relations or migrate them safely.
- Upgrades MUST be tested against an existing installed state, not only on a fresh database.

Required update sequence:

1. Bump the module version.
2. Add and validate migrations.
3. Update manifest declarations for permissions, menus, routes, settings, events, and hooks.
4. Run the module through the upgrade lifecycle.
5. Revalidate existing records, attachments, and permissions after upgrade.

### 23.7 Module Removal And Compatibility Rules

Modules must be designed so they can be upgraded or removed without breaking the platform.

- Never hardcode module-specific assumptions in `packages/*`.
- Never require another module through a direct import.
- Business records in one module may reference another module only through events, SDK contracts, or structured external identifiers.
- File attachments owned by a module MUST be unlinkable or migratable during uninstall.
- UI components placed in shell slots, menus, command surfaces, and dashboards MUST disappear cleanly when the module is removed.
