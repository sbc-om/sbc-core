    ...
    >
      {queues.length === 0 ? (
        <CallCenterEmptyState
          title="No queues yet"
          description="Start by adding a service queue for support, VIP, outbound, or escalation workflows."
        />
      ) : (
        <div className="grid gap-4 lg:grid-cols-2">
          {queues.map((queue) => (
            <article key={queue.code} className="rounded-lg border border-border p-5">
