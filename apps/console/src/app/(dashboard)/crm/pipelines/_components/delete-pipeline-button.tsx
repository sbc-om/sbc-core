...
import { deletePipelineAction } from "@/actions/crm";
...
export function DeletePipelineButton({ id, name }: { id: string; name: string }) {
  ...
  async function handle() {
    const ok = await confirm({
      title:        `Delete "${name}"?`,
      description:  "This will remove the pipeline. Leads assigned to this pipeline will not be deleted.",
      confirmLabel: "Delete",
      tone:         "danger",
    });
    if (!ok) return;
    startTransition(async () => {
      ...
      else              toast.success("Pipeline deleted", `"${name}" has been removed.`);
    });
  }
